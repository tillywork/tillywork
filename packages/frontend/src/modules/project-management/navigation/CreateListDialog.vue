	...
	<v-dialog v-model="createListDialog" width="400">
		<v-form ref="createListForm" @submit.prevent="createList">
			<v-card :loading="createListLoading">
				...
				<v-card-title class="text-body-2 font-weight-medium">Create List in {{ selectedSpace?.name }}</v-card-title>
				<v-divider />
				<v-card-text>
					<v-text-field v-model="createListData.name" prepend-inner-icon="mdi-file-cabinet" single-line
						label="List Name" :rules="[validation.rules.required]" />
				</v-card-text>
				<v-divider />
				<v-card-actions>
					<v-spacer />
					<v-btn color="default" @click="closeCreateListDialog()" :disabled="createListLoading" rounded="xl" class="text-capitalize">Cancel</v-btn>
					<v-btn variant="flat" :disabled="createListLoading" type="submit" rounded="xl" class="text-capitalize">Create</v-btn>
				</v-card-actions>
			</v-card>
		</v-form>
	</v-dialog>
