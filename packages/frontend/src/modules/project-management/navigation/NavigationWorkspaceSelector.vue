...
<template>
    <div class="d-flex justify-center align-center mt-2 px-2">
        <v-card density="compact" link id="workspace-menu-activator" color="background" class="flex-1-0 me-2">
            <v-card-title class="d-flex align-center text-body-2 font-weight-bold">
                ...
                <v-spacer />
                <v-icon>{{ selectWorkspaceMenu ? 'mdi-chevron-up' : 'mdi-chevron-down' }}</v-icon>
            </v-card-title>
        </v-card>

        <v-btn id="workspace-menu-btn" variant="text" color="default" density="compact" icon="mdi-dots-horizontal" rounded="md" size="small"
            @click.stop />
    ...
    <v-menu v-model="selectWorkspaceMenu" :close-on-content-click="false" activator="#workspace-menu-activator">
        <v-card class="pt-3 mt-2" width="300px" density="compact">
            <div class="px-5 text-truncate mb-2">
                <v-icon size="small">mdi-sitemap</v-icon>
                <span class="ml-1">
