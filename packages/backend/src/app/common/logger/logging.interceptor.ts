...
export class LoggingInterceptor implements NestInterceptor {
    ...
    private logResponse(request: FastifyRequest, response: FastifyReply, startTime: [number, number]) {
        const responseTime = process.hrtime(startTime);
        const responseTimeMs = responseTime[0] * 1000 + responseTime[1] / 1e6;

        this.logger.log(`${request.id} - ${request.method} ${request.url} - ${response.statusCode} - ${responseTimeMs.toFixed(2)}ms`);
    }

    private logError(request: FastifyRequest, response: FastifyReply, startTime: [number, number], error: Error) {
        const responseTime = process.hrtime(startTime);
        const responseTimeMs = responseTime[0] * 1000 + responseTime[1] / 1e6;

        this.logger.error(
            `${request.id} - ${request.method} ${request.url} - ${response.statusCode || 500} - ${responseTimeMs.toFixed(
                2,
            )}ms`,
            `Error: ${error.message}`
        );
